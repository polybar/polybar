...
POLYBAR_NS
class pulseaudio_sink : public pulseaudio {
  // events to add to our queue
  enum class evtype { NEW = 0, CHANGE, REMOVE, SERVER };
  using queue = std::queue<evtype>;

 public:
  explicit pulseaudio_sink(const logger& logger, string&& sink_name, bool m_max_volume);
  virtual ~pulseaudio_sink();

  pulseaudio_sink(const pulseaudio_sink& o) = delete;
  pulseaudio_sink& operator=(const pulseaudio_sink& o) = delete;
